...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.CORTEX_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.CORTEX_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.CORTEX_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".cortex") && dir !== Flag.CORTEX_CONFIG_DIR) continue
