...
const managedConfigDir = process.env.CORTEX_TEST_MANAGED_CONFIG_DIR!
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["CORTEX_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      opencode: {
        options: {
          apiKey: "{env:CORTEX_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["CORTEX_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["CORTEX_CONSOLE_TOKEN"]
    }
  }
})
...
test("does not try to install dependencies in read-only CORTEX_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  process.env.CORTEX_CONFIG_DIR = tmp.extra
...
test("installs dependencies in writable CORTEX_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
// Note: preload.ts sets CORTEX_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("CORTEX_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["CORTEX_DISABLE_PROJECT_CONFIG"]
    process.env["CORTEX_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["CORTEX_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["CORTEX_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["CORTEX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CORTEX_CONFIG_DIR"]
    ...
    try {
      ...
      if (originalDisable === undefined) {
        ...
        process.env["CORTEX_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["CORTEX_CONFIG_DIR"]
      } else {
        process.env["CORTEX_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("CORTEX_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["CORTEX_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CORTEX_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["CORTEX_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["CORTEX_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from CORTEX_CONFIG_DIR, not project
...
describe("CORTEX_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in CORTEX_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CORTEX_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["CORTEX_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["CORTEX_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["CORTEX_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in CORTEX_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CORTEX_CONFIG_CONTENT"]
