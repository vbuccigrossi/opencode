...
afterEach(async () => {
  ...
  delete process.env.CORTEX_TUI_CONFIG
...
test("project config takes precedence over CORTEX_TUI_CONFIG (matches CORTEX_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.CORTEX_TUI_CONFIG = custom
    },
  })
...
test("CORTEX_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from CORTEX_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.CORTEX_CONFIG = path.join(customDir, "opencode.json")
    },
  })
