...
# ============================================================================
# OpenCode Workstation Installer
#
# One-script setup for an offline-capable security research coding agent.
# Installs: bun, ollama, models, opencode, global config, RAG docs.
#
# Usage:
#   bash install-opencode-workstation.sh [OPTIONS]
#
# Options:
...
OPENCODE_DIR="$HOME/Desktop/projects/opencode"
BUN_DIR="$HOME/.bun"
...
echo ""
echo "============================================"
echo "  OpenCode Workstation Installer"
echo "============================================"
echo ""
...
info "OpenCode dir:     $OPENCODE_DIR"
echo ""
...
# ── Step 5: Clone/update opencode ──

info "[5/8] Setting up opencode repository..."

if [[ -d "$OPENCODE_DIR/.git" ]]; then
    ok "opencode repo exists at $OPENCODE_DIR"
    info "Pulling latest changes..."
    cd "$OPENCODE_DIR" && git pull --ff-only 2>/dev/null || warn "git pull failed (may have local changes)"
else
    info "Cloning opencode..."
    mkdir -p "$(dirname "$OPENCODE_DIR")"
    git clone "$OPENCODE_REPO" "$OPENCODE_DIR"
    ok "opencode cloned to $OPENCODE_DIR"
fi
...
#   opencode -m ollama/model    — start with specific model
#   opencode run "message"      — non-interactive mode

OPENCODE_ROOT="PLACEHOLDER_OPENCODE_ROOT"
BUN="PLACEHOLDER_BUN"
PROJECT_DIR="$(pwd)"
...
case "$1" in
  run|serve|web|attach)
    cd "$OPENCODE_ROOT" && exec "$BUN" run --conditions=browser ./src/index.ts "$@" --dir "$PROJECT_DIR"
    ;;
  *)
    cd "$OPENCODE_ROOT" && exec "$BUN" run --conditions=browser ./src/index.ts "$PROJECT_DIR" "$@"
    ;;
esac
LAUNCHER_EOF

# Replace placeholders with actual paths
sed -i "s|PLACEHOLDER_OPENCODE_ROOT|${OPENCODE_DIR}/packages/opencode|" "$LAUNCHER"
...
EOF
ok "Global config written to $CONFIG_DIR/opencode.jsonc"
...
if [[ "$SKIP_DOCS" == true ]]; then
    ...
    warn "  python3 $OPENCODE_DIR/scripts/build-go-docs.py --output $WORK_DIR/.docs/ --all-stdlib --error-patterns"
else
    ...
    python3 "$OPENCODE_DIR/scripts/build-go-docs.py" \
        --output "$DOCS_DIR" \
        --all-stdlib \
        --error-patterns \
        2>&1 | tail -5
...
EOFDOC
    ...
    rm -f "$HOME/.local/share/opencode/rag-manifest.json"
...
echo "  Instructions:   ~/.config/opencode/instructions.md"
echo "  Launcher:       ~/.local/bin/opencode"
...
echo "  Project-level opencode.jsonc files override it if present."
echo ""
